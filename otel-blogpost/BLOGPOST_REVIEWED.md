...
Before jumping into implementation details, it helps to be explicit about the *shape* of telemetry you want from an agent.

KAOS emits spans that mirror the agentic loop:

- **`agent.agentic_loop`**: one span per `/v1/chat/completions` request (root of the trace for that agent)
- **`agent.step.N`**: one span per reasoning iteration
- **`model.inference`**: the LLM call (duration + model metadata)
- **`tool.<name>` / `mcp.tool.<name>`**: tool execution (duration + tool name + status)
- **`delegate.<agent>`**: outbound call to another agent (duration + target agent)

A good rule of thumb: **span names are stable; variability goes into attributes**.

For example:

- ✅ `tool.web_search` with `tool.args.preview="query=..."`  
- ❌ `tool.web_search?query=what-is-open-telemetry` (high-cardinality span names)

### Noise vs Signal: Auto-Instrumentation Trade-offs

OpenTelemetry makes it tempting to auto-instrument *everything* (FastAPI + HTTP clients) and call it a day. With agents, that can backfire:

- agent-to-agent calls can explode into nested HTTP spans that add little value on top of your higher-level `delegate.*` spans
- MCP servers often use long-lived streaming connections (SSE), which can create “forever spans” and drown out the meaningful ones
- Kubernetes probes (`/health`, `/ready`) can generate constant trace noise

KAOS defaults to **high-signal manual spans** for agent operations, and makes lower-level HTTP auto-instrumentation **opt-in** when you actually need it. You get predictable traces in production, and you can still flip the switches when debugging:

- Enable HTTP server spans when you want request-level visibility (`OTEL_INCLUDE_HTTP_SERVER=true`)
- Enable HTTP client spans when you want raw downstream timing (`OTEL_INCLUDE_HTTP_CLIENT=true`)
- Exclude health checks from tracing (FastAPI excluded URLs)

### The “Log Correlation” Footgun

Log/trace correlation isn’t just about exporting logs. It also depends on *when* you log.

If you log after your span has been marked as failed/ended—and your code detaches context—you’ll see error logs with **no `trace_id`**, which breaks correlation.

The safe pattern is:

1. create span  
2. log while context is active  
3. mark span failure / end span  

We’ll come back to the concrete code pattern below.

### Memory Events as Trace Breadcrumbs

KAOS keeps durable “memory events” (session state, tool results, delegation summaries). A practical observability trick is to store the *current* trace context (trace_id/span_id) inside those memory events.

That gives you two additional workflows:

- start from a trace → jump into memory history for that session
- start from a memory event → find the exact trace that produced it

It’s the agent equivalent of “business events with trace IDs” in a traditional distributed system.
...
## Challenges and Nuances in Production

Once you have “hello world” telemetry working, the hard problems are mostly about *scale* and *operability*.

### 1) Cardinality: the fastest way to break your metrics backend

Agent systems naturally produce unbounded identifiers: session IDs, user IDs, tool arguments, prompt text, URLs, file names.

- Put high-cardinality values in **logs** or **trace attributes**.
- Keep **metric labels** low-cardinality (`agent.name`, `tool.name`, `status`, `model`).

If you want per-customer or per-session views, use traces/logs + backend queries—not metric labels.

### 2) Privacy & security: prompts are production data

Observability backends are often broadly accessible inside orgs. Full prompts and tool outputs may contain secrets, PII, or proprietary data.

Pragmatic approach:

- log **previews**, lengths, hashes, and structured metadata
- keep full content in systems designed for it (secure storage, encrypted memory, controlled access)
- add explicit redaction for known sensitive keys (`api_key`, `Authorization`, cookies)

### 3) Sampling: long traces are expensive traces

A single agent request can produce dozens of spans (multi-step loops + tools + delegation). At scale, always-on full-fidelity tracing can become cost-prohibitive.

Two common strategies:

- **Head-based sampling** (SDK decides at the start): cheap, but can miss the interesting slow/error traces.
- **Tail-based sampling** (collector decides after seeing the whole trace): ideal for agents because you can keep *all errors* and *slow traces* while dropping most successful fast ones.

If you’re using an OpenTelemetry Collector, tail sampling is often the best “first upgrade” after the demo.

### 4) Naming conventions: stable span names beat clever span names

You’ll thank yourself later if you keep span names stable and encode variability as attributes. It makes dashboards, alerts, and SLOs dramatically simpler.

A simple convention that works well:

- `agent.*` for loop/step spans  
- `model.*` for inference  
- `tool.*` / `mcp.tool.*` for tool execution  
- `delegate.*` for cross-agent calls  

### 5) GenAI semantic conventions are evolving

OpenTelemetry’s semantic conventions for GenAI are still maturing. Treat them like a target you can gradually align to:

- start with stable, internal attributes (`agent.name`, `tool.name`, `session.id`)
- add `gen_ai.*` attributes where they’re already useful (model name, finish reason, token usage)
- version your attribute schema (so you can migrate without breaking dashboards)
...
## Resources
- **OpenTelemetry Collector**: The vendor-neutral pipeline for receiving OTLP and exporting to your backend.
- **W3C Trace Context**: The standard behind `traceparent`/`tracestate` header propagation.
- **KAOS Telemetry Docs**: Operator-driven configuration, env var propagation, and noise controls (LOG_LEVEL, OTEL_INCLUDE_HTTP_*).
